...
** Table of Contents :TOC_3_gh:
  ...
  - [[#org][ORG]]
    ...
    - [[#org-super-agenda-keybinding][Org Super Agenda Keybinding]]
...
(set-frame-size (selected-frame) 1240 1378 t)
...
;;C, C++ 관련 문서화를 할때 코드의 주석 스타일을 통일한다.
 ;; (use-package gendoxy)
#+END_SRC
...
(after! org
  ...
  (setq org-capture-templates
                  '(("s" "Personal Schedule" entry
                    (file "~/OneDrive/org/personal_schedule.org")
                    "* %?\nSCHEDULED: %t\n :PROPERTIES:\n :agenda-group: PERSONAL\n :END:\n"
                    :prepend t :kill-buffer t)
                    ("t" "Personal TODO" entry
                    (file "~/OneDrive/org/tasks.org")
                    "* TODO%?\nDEADLINE: %t\n :PROPERTIES:\n :agenda-group: PERSONAL\n :END:\n"
                    :prepend t :kill-buffer t)
                    ("j" "JW TODO" entry
                    (file "~/OneDrive/org/jw-todos.org")
                    "* TODO %?\nDEADLINE: %t\n :PROPERTIES:\n :agenda-group: JW.ORG\n :END:\n"
                    :prepend t :kill-buffer t)
                    ("m" "프로임명" entry
                    (file "~/OneDrive/org/jw.org")
                    "* %?\nSCHEDULED: %t\n :PROPERTIES:\n :agenda-group: JW.ORG\n :END:\n"
                    :prepend t :kill-buffer t)
                    ("b" "BRV일정" entry
                    (file "~/OneDrive/org/bethel.org")
                    "* %?\nSCHEDULED: %t\n :PROPERTIES:\n :agenda-group: JW.ORG\n :END:\n"
                    :prepend t :kill-buffer t)
                    ("B" "BRV TODO" entry
                    (file "~/OneDrive/org/bethel.org")
...
(defun my/style-org-agenda()
  ...
  (set-face-attribute 'org-super-agenda-header nil :foreground "lightgreen")
...
그리고 설정 파일에서 저장된 파일을 다시 읽어 들이도록 한다.
#+begin_src elisp :tangle config.el
